...
rhoNorm(1) = 1/D0 * ( (6*(D(3,1)*tau(1)/tau(3) + D(2,1)*tau(2)/tau(3))*r2Norm^3  ...
    + mu*D(3,1)*(tau(2)^2 - tau(1)^2)*tau(1)/tau(3) ) ...
    / (6*r2Norm^3 + mu*(tau(2)^2 - tau(3)^2)) - D(1,1) ) ;
rhoNorm(2) = A + mu*B/r2Norm^3;
rhoNorm(3) = 1/D0 * ( (6*(D(1,3)*tau(3)/tau(1) - D(2,3)*tau(2)/tau(1))*r2Norm^3  ...
    + mu*D(1,3)*(tau(2)^2 - tau(3)^2)*tau(3)/tau(1) ) ...
